...
public class DF : MonoBehaviour
{
	...
	public static void Log(object str, E_ColorType c = E_ColorType.Normal)
	{
		...
		{
			formatStr = "<color=#C94A4A>----------" + str + "----------------</color>";//FF0000  C94A4A
			Debug.Log(formatStr);
		}
	...
	/*
	public static void RecordLog(bool once=true)
    {
		if(!isRecord)
        {
			Log("record is closed !!!", E_ColorType.Err);
			return;
        }
		string path = Application.dataPath;
		FileStream fs = null;
		string fileName = DFHelper.ToolTime.GetFormatDateTime(null,E_DateType.Log);
		string fullPath = path + "/Log/" + fileName + "_log.txt";
